...
<div class="row align-items-center">
    <div class="col-sm-6">
        <h3 class="mb-0 fw-semibold text-dark">Liste des types de contenus</h3>
    </div>
...
<div class="card border-0 shadow-sm" style="border-radius: 12px;">
    ...
    <div class="card-body p-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h5 class="mb-1 fw-semibold text-dark">Gestion des types de contenus</h5>
                <p class="text-muted small mb-0">{{ $typecontenus->total() }} type(s) de contenu au total</p>
            </div>
            ...
            <div class="ms-3" style="flex: 0 0 auto;">
                <a href="{{ route('typecontenus.create') }}"
                   class="btn btn-success d-flex align-items-center gap-2"
                   style="border-radius: 8px; padding: 10px 20px;">
                    <i class="bi bi-plus-lg"></i>
                    <span>Ajouter un type de contenu</span>
                </a>
            </div>
        </div>
            ...
            <table class="table table-hover table-bordered align-middle" style="border-collapse: collapse;">
                 ...
                 <thead>
                     <tr style="background-color: #f8f9fa; border-bottom: 2px solid #dee2e6;">
                        <th class="border-0 py-3 ps-3 col-md-8" style="border-radius: 8px 0 0 0;">
                            <span class="text-uppercase fw-semibold text-primary" style="font-size: 0.75rem; letter-spacing: 0.5px;">Nom</span>
                        </th>
                        <th class="border-0 py-3 text-center pe-3 col-md-4" style="border-radius: 0 8px 0 0;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid #f1f3f5;">
                        <td class="ps-3 py-3">
                            <span class="fw-medium text-dark">{{ $typecontenu->nom }}</span>
                        </td>
                        
                        <td class="text-center pe-3 py-3">
                            <div class="d-inline-flex gap-2">
                                <!-- Bouton Voir -->
                                <a href="{{ route('typecontenus.show', $typecontenu->id) }}" 
                                ...
                                <!-- Bouton Modifier -->
                                <a href="{{ route('typecontenus.edit', $typecontenu->id) }}" 
                                ...
                                <!-- Bouton Supprimer -->
                                <form action="{{ route('typecontenus.destroy', $typecontenu->id) }}" 
                    ...
                    <tr>
                        <td colspan="2" class="text-center py-5">
                            <div class="d-flex flex-column align-items-center">
                                ...
                                <p class="text-indigo-muted mb-0">Aucun type de contenu trouvé</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <!-- Pagination -->
        @if($typecontenus->hasPages())
        <div class="d-flex justify-content-between align-items-center mt-4 pt-3" style="border-top: 1px solid #f1f3f5;">
            <div class="text-muted small">
                Affichage de <strong>{{ $typecontenus->firstItem() ?? 0 }}</strong> à <strong>{{ $typecontenus->lastItem() ?? 0 }}</strong> sur <strong>{{ $typecontenus->total() }}</strong>
            </div>
            <div>
                {{ $typecontenus->links('pagination::bootstrap-5') }}
            </div>
        </div>
        @endif
