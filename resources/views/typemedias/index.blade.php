...
<div class="row align-items-center">
    <div class="col-sm-6">
        <h3 class="mb-0 fw-semibold text-dark">Liste des types de médias</h3>
    </div>
...
<div class="card border-0 shadow-sm" style="border-radius: 12px;">
    ...
    <div class="card-body p-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h5 class="mb-1 fw-semibold text-dark">Gestion des types de médias</h5>
                <p class="text-muted small mb-0">{{ $typemedias->total() }} type(s) de média au total</p>
            </div>
            ...
            <div class="ms-3" style="flex: 0 0 auto;">
                <a href="{{ route('typemedias.create') }}"
                   class="btn btn-success d-flex align-items-center gap-2"
                   style="border-radius: 8px; padding: 10px 20px;">
                    <i class="bi bi-plus-lg"></i>
                    <span>Ajouter un type de média</span>
                </a>
            </div>
        </div>
            ...
            <table class="table table-hover table-bordered align-middle" style="border-collapse: collapse;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid #f1f3f5;">
                        <td class="ps-3 py-3">
                            <span class="fw-medium text-dark">{{ $typemedia->nom }}</span>
                        </td>
                        
                        <td class="text-center pe-3 py-3">
                            <div class="d-inline-flex gap-2">
                                <!-- Bouton Voir -->
                                <a href="{{ route('typemedias.show', $typemedia->id) }}" 
                                ...
                                <!-- Bouton Modifier -->
                                <a href="{{ route('typemedias.edit', $typemedia->id) }}" 
                                ...
                                <!-- Bouton Supprimer -->
                                <form action="{{ route('typemedias.destroy', $typemedia->id) }}" 
                    ...
                    <tr>
                        <td colspan="2" class="text-center py-5">
                            <div class="d-flex flex-column align-items-center">
                                ...
                                <p class="text-indigo-muted mb-0">Aucun type de média trouvé</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="d-flex justify-content-between align-items-center mt-4 pt-3" style="border-top: 1px solid #f1f3f5;">
            <div class="text-muted small">
                Affichage de <strong>{{ $typemedias->firstItem() ?? 0 }}</strong> à <strong>{{ $typemedias->lastItem() ?? 0 }}</strong> sur <strong>{{ $typemedias->total() }}</strong>
            </div>
            <div>
                {{ $typemedias->links('pagination::bootstrap-5') }}
            </div>
        </div>
        @endif
